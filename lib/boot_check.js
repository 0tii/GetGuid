...
export async function bootCheck() {
    ...
    if (cfg.default.useSSL) {
        ...
        if (!cfg.default.certName) {
            if(cfg.default.certFiles){
                console.error("Certificate files expected, but no file names provided.");
                process.exit(1);
            }
        ...
        try {
            await import (`../sslcert/${cfg.default.certName}`);
        } catch {
            if(cfg.default.certFiles){
                console.error("Certificate file name has been provided, but no corresponding file could be found.");
                process.exit(1);
            }
            console.warn("SSL is enabled but certificate can not be found in /sslcert.");
            warn = true;
        }

        try {
            await import (`../sslcert/${cfg.default.privKeyName}`);
        } catch {
            if(cfg.default.certFiles){
                console.error("Certificate private key name has been provided, but no corresponding file could be found.");
                process.exit(1);
            }
            console.warn("SSL is enabled but private key can not be found in /sslcert.");
            warn = true;
        }
