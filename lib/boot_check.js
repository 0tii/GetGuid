...
export async function bootCheck() {
    ...
    if (cfg.useSSL) {
        ...
        if (!cfg.certName) {
            console.log("[Warning] SSL is enabled but certificate file name has not been specified in config.js");
            warn = true;
        }

        if (!cfg.privKeyName) {
            console.log("[Warning] SSL is enabled but private key file name has not been specified in config.js");
            warn = true;
        }

        try {
            await
            import (`../sslcert/${cfg.certName}`);
        } catch {
            console.log("[Warning] SSL is enabled but certificate can not be found in /sslcert.");
            warn = true;
        }

        try {
            await
            import (`../sslcert/${cfg.privKeyName}`);
        } catch {
            console.log("[Warning] SSL is enabled but private key can not be found in /sslcert.");
            warn = true;
        }

        if (Warn)
            console.log("[Warning | SSL] One or more certificate files are missing or unspecified. You can ignore this if you secured your traffic by other means such as VirtualHost Proxy.");
    }
}
